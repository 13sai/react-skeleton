...
export default memo(({ jsonMenuData, routeItem }: RightTabNavProps) => {
  ...
  const moveToView = (index: number): void => {
    ...
    const tabItemEl = scrollContentRef.current.childNodes[
      index
    ] as HTMLSpanElement;
    if (!tabItemEl) {
      return;
    ...
    if (contentWidth < boxWidth || tabItemElOffsetLeft === 0) {
      ...
      tabItemElOffsetLeft > -translateX &&
      tabItemElOffsetLeft + tabItemOffsetWidth < -translateX + boxWidth
    ) {
      // 标签在可视区域
      setTranslateX(
        Math.min(
          0,
          boxWidth - tabItemOffsetWidth - tabItemElOffsetLeft - tabNavPadding
        )
      );
    } else {
      // 标签在可视区域右侧
      setTranslateX(
        -(tabItemElOffsetLeft - (boxWidth - tabNavPadding - tabItemOffsetWidth))
      );
    }
  };
  ...
  const closeTabNavLeftRight = (param: 'left' | 'right'): void => {
    // 获取当前打开tabNav索引
    const index = tabNavList.findIndex((item) =>
      equalTabNavRoute(location, item.location, item.menu.meta?.tabNavType)
    );
